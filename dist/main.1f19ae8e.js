...
function handleResult({
  ...
  color = color.replace(/\s/g, ""); // check if it's a valid color

  if (!(0, _colors.isValidColor)(color)) return; // than show the UI for it

  const colorClass = document.querySelector(`.${color}`);
  colorClass.classList.add('got'); // change the background color
...
},{"./colors.js":"colors.js"}],"main.js":[function(require,module,exports) {
"use strict";
...
function displayColors(colors) {
  return colors.map(color => `<div class="color ${color} ${(0, _colors.isDark)(color) ? 'dark' : ''}" style="background: ${color}">${color}</div>`).join('');
}
...
function start() {
  ...
  console.log('Starting speech recognition...'); // make a new speech recognition

  const recognition = new SpeechRecognition();
  recognition.continuous = true;
  recognition.interimResults = true; // different event listener for SpeechRecognition
